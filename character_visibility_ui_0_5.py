...
class CHVI_visibility_group(bpy.types.PropertyGroup):
    ...
    def viewport_toggle(self, context):  
        for item in self.items:
            if item.is_mod:
                if item.mesh_p.modifiers[item.mod_p].type != 'MASK':
                    item.mesh_p.modifiers[item.mod_p].show_viewport = self.visibility
                else:
    ...
    def render_toggle(self, context):  
        for item in self.items:
            if item.is_mod:
                if item.mesh_p.modifiers[item.mod_p].type != 'MASK':
                    item.mesh_p.modifiers[item.mod_p].show_render = self.rendering
                else:
